...
import org.intellij.plugins.native2Debugger.impl.Native2BreakpointHandler;
...
/**
 * This is the main place that interacts with the debugged NATIVE2 processor. Waits until the processor
 * hits a breakpoint, resumes execution, etc.
 */
public final class Native2DebuggerSession implements Disposable {
  private static final Logger LOG = Logger.getInstance(Native2DebuggerSession.class);

  private static final Key<Native2DebuggerSession> DEBUGGER_SESSION = Key.create("DEBUGGER_SESSION");
  ...
  private Native2DebuggerSession(Project project, ProcessHandler process, Debugger client) {
    myProject = project;
    myProcess = process;
    myClient = client;
    Disposer.register(Native2DebugProcess.getInstance(process), this);
  }
  ...
  public boolean canRunTo(final XSourcePosition position) {
    return Native2BreakpointHandler.getActualLineNumber(myProject, position) != -1;
  }

  public void runTo(final PsiFile file, final XSourcePosition position) {
    assert myTempBreakpoint == null;

    final int lineNumber = Native2BreakpointHandler.getActualLineNumber(myProject, position);
    final String uri = Native2BreakpointHandler.getFileURL(file.getVirtualFile());
  ...
  @NotNull
  public static Native2DebuggerSession create(Project project, @NotNull ProcessHandler process, Debugger client) {
    final Native2DebuggerSession session = new Native2DebuggerSession(project, process, client);
    process.putUserData(DEBUGGER_SESSION, session);
    return session;
  }

  public static Native2DebuggerSession getInstance(@NotNull ProcessHandler process) {
